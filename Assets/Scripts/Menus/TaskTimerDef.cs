...
public class TaskTimerDef : MonoBehaviour
{
    private float taskStartTime;
    private float taskEndTime;
    private bool isTiming;
    private int taskCount;
    private List<string> currentTaskRespostas; // Variável de membro para armazenar as respostas da tarefa atual.

    private List<TaskDefData> taskDataList = new List<TaskDefData>();
    private string folderPath;
    private string fileNameBase = "task_defilade";
    ...
    void StartNewTask()
    {
        ...
        // Cria uma nova lista de respostas para a tarefa atual.
        currentTaskRespostas = new List<string>();
    }
    ...
    void EndCurrentTask()
    {
        ...
        // Adiciona a resposta à lista de respostas da tarefa atual.
        currentTaskRespostas.Add(respostaSelecionada);

        // Limpa os tempos da tarefa atual.
        taskStartTime = 0f;
        taskEndTime = 0f;

        // Salva os dados das respostas na nova estrutura TaskDefData.
        TaskDefData taskDefData = new TaskDefData(currentTaskName, currentTaskRespostas, taskStartTime, taskEndTime);
        taskDataList.Add(taskDefData);
    ...
    void SaveTaskDataToCSV()
    {
        ...
        foreach (TaskDefData taskDefData in taskDataList)
        {
            float totalTaskTime = taskDefData.endTime - taskDefData.startTime;
            string csvLine = taskDefData.taskName + "," + string.Join("|", taskDefData.respostas) + "," + taskDefData.startTime + "," + taskDefData.endTime + "," + totalTaskTime;
...
public struct TaskDefData
{
    ...
    public TaskDefData(string name, List<string> responses, float start, float end)
    {
        taskName = name;
        respostas = responses;
        startTime = start;
