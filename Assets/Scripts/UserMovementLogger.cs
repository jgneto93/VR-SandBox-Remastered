...
public class UserMovementLogger : MonoBehaviour {
    ...
    public void LogMovement(int currentFrame,Transform transform) {
        if (lastTransform != transform ||
            ...
            string logEntry = $"{Time.unscaledTime}; {currentFrame}; {transform.position}; {transform.rotation}";
    ...
    public void CreateNewTestLogFile() {
        try {
            ...
            string newLogFilePath = Path.Combine(Application.persistentDataPath, $"UserMovementLog{logFileIndex}_{dateTimeString}.txt");
